"""
Inference script for anomaly_type model
Usage: python postpaid/inferences/predict_anomaly_type.py --final_score_rumus1 20 --final_score_rumus2 10
"""
...
def predict(features: dict, model_dir: str = None) -> dict:
    """
    Make prediction using trained anomaly_type model.

    Args:
        features: Dictionary of feature names and values
                 Example: {'final_score_rumus1': 20, 'final_score_rumus2': 10}
    ...
    predicted_type = label_mapping.get(str(prediction_encoded), prediction_encoded)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Predict anomaly root cause type'
    )
    parser.add_argument(
        '--final_score_rumus1',
        type=float,
        required=True,
        help='Severity score from final_score_rumus1 (anomaly persistence)'
    )
    parser.add_argument(
        '--final_score_rumus2',
        type=float,
        required=True,
        help='Severity score from final_score_rumus2 (gardu deviation)'
    )
    ...
    # Make prediction
    print("="*60)
    print("ANOMALY ROOT CAUSE CLASSIFICATION")
    print("="*60)
    print(f"\nInput features:")
    print(f"  - final_score_rumus1 (persistence severity): {features['final_score_rumus1']}")
    print(f"  - final_score_rumus2 (gardu deviation severity): {features['final_score_rumus2']}")
    ...
    for class_label, prob in result['probabilities'].items():
        print(f"  - {class_label.upper()}: {prob:.2%}")

    print(f"\nFinal Prediction:")
    print(f"  - Root Cause: {result['predicted_type'].upper()}")
    print(f"  - Confidence: {result['confidence']:.2%}")

    # Interpretation
    anomaly_type = result['predicted_type']
    print(f"\nInterpretation:")
    if anomaly_type == 'none':
        print(f"  - Type: NO ANOMALY")
        print(f"  - Both persistence and gardu deviation scores are normal")
        print(f"  - No action needed")
    elif anomaly_type == 'baseline':
        print(f"  - Type: BASELINE ANOMALY")
        print(f"  - Issue with customer-specific baseline/consumption")
        print(f"  - Action: Check customer consumption history and adjust baseline")
    elif anomaly_type == 'gardu':
        print(f"  - Type: GARDU ANOMALY")
        print(f"  - Issue at gardu/substation level")
        print(f"  - Action: Investigate gardu infrastructure and area issues")
    elif anomaly_type == 'both':
        print(f"  - Type: COMBINED ANOMALY")
        print(f"  - Issues detected at both customer and gardu levels")
        print(f"  - Action: Comprehensive investigation required")
