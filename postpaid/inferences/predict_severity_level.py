"""
Inference script for severity_level model
Usage: python postpaid/inferences/predict_severity_level.py --final_score_rumus1 20 --final_score_rumus2 30
"""
...
def predict(features: dict, model_dir: str = None) -> dict:
    """
    Make prediction using trained severity_level model.

    Args:
        features: Dictionary of feature names and values
                 Example: {'final_score_rumus1': 20, 'final_score_rumus2': 30}
    ...
    predicted_level = label_mapping.get(str(prediction_encoded), prediction_encoded)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Predict final severity level for anomaly'
    )
    parser.add_argument(
        '--final_score_rumus1',
        type=float,
    ...
    # Make prediction
    print("="*60)
    print("FINAL SEVERITY LEVEL PREDICTION")
    print("="*60)
    print(f"\nInput features:")
    ...
    print(f"\nFinal Prediction:")
    print(f"  - Severity Level: {result['predicted_level'].upper()}")
    ...
    if severity == 'low':
        print(f"  - 🔵 Level: LOW")
        print(f"  - Minor anomaly detected")
        print(f"  - Action: Routine monitoring")
        print(f"  - Priority: P4 (Low)")
    elif severity == 'medium':
        print(f"  - 🟢 Level: MEDIUM")
        print(f"  - Moderate anomaly detected")
        print(f"  - Action: Increased monitoring required")
        print(f"  - Priority: P3 (Medium)")
    elif severity == 'high':
        print(f"  - 🟡 Level: HIGH")
        print(f"  - Significant anomaly detected")
        print(f"  - Action: Investigation recommended")
        print(f"  - Priority: P2 (High)")
    elif severity == 'critical':
        print(f"  - 🔴 Level: CRITICAL")
        print(f"  - Severe anomaly detected")
